@section('title', 'Admin Indicadores')
...
<div>
              ...
              <tr>
                ...
                <td class="align-middle">
                  <div class="d-flex flex-md-row gap-1 justify-content-evenly">
                    <div class="m-1 mt-3">
                      <livewire:toggle-button :model="$indicador" field="estado" key="{{ $indicador->id }}" />
                    </div>
                    <button wire:click="edit({{ $indicador->id }})" class="btn btn-sm btn-primary m-1"
                      data-toggle="modal" data-target="#roleModal" title="Editar"><i class="fa fa-edit"></i></button>
                    <button wire:click="$emit('alertDelete',{{ $indicador->id }})" class="btn btn-sm btn-danger m-1"
  ...
  {{--    Mostrar modal para agregar o editar indices --}}
  @if ($showModal == 'block')
    @include('livewire.admin.indices-form')
  @endif
</div>
